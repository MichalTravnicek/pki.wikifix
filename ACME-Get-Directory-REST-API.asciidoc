= Overview =

The PKI ACME Directory operation will return the ACME directory as defined in link:https://tools.ietf.org/html/rfc8555#section-7.1.1[RFC 8555 Section 7.1.1].

= Request =

* *URL:* `/acme/directory`
* *Method:* `GET`
* *Authentication:* Not required
* *Authorization:* Not required

= Response =

== Success ==

* *Code:* `200`
* *Content*:

----
{
    "meta": {
        "caaIdentities": [
            ...
        ],
        "externalAccountRequired": ...,
        "termsOfService": ...,
        "website": ...
    },
    "newAccount": ...,
    "newNonce": ...,
    "newOrder": ...,
    "revokeCert": ...
}
----

== Unavailable ==

* *Code:* `503`

= Examples =

.JSON
[%collapsible]
====
----
$ curl \
    -s \
    https://pki.demo.dogtagpki.org/acme/directory | python -m json.tool
{
    "newNonce": "https://pki.demo.dogtagpki.org/acme/new-nonce",
    "newAccount": "https://pki.demo.dogtagpki.org/acme/new-account",
    "newOrder": "https://pki.demo.dogtagpki.org/acme/new-order",
    "revokeCert": "https://pki.demo.dogtagpki.org/acme/revoke-cert",
    "meta": {
        "termsOfService": "https://github.com/dogtagpki/pki/wiki/ACME-Responder-Demo",
        "website": "https://www.dogtagpki.org",
        "caaIdentities": [
            "dogtagpki.org"
        ],
        "externalAccountRequired": false
    }
}
----
====

= See Also =

* link:ACME-REST-API[ACME REST API]
