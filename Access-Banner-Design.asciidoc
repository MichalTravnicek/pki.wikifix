...
To install banner on a PKI server instance, store the content of the banner in `banner.txt` in the instance’s configuration folder (i.e. `/etc/pki/<instance>`), for example:
...
* Root page: http://server.example.com:8080/
* PKI UI welcome page: http://server.example.com:8080/pki/
* PKI UI main page: https://server.example.com:8443/pki/ui/
* PKI subsystem UIs:
** https://server.example.com:8443/ca/
** https://server.example.com:8443/ka/
** https://server.example.com:8443/ocsp/
** https://server.example.com:8443/tks/
** https://server.example.com:8443/tps/
...
----
$ pki -d ~/.dogtag/pki-tomcat/ca/alias -n caadmin -c Secret.123 ca-user-find
WARNING!
Access to this service is restricted to those individuals with
specific permissions. If you are not an authorized user, disconnect
now. Any attempts to gain unauthorized access will be prosecuted to
