...
Each PKI instance can be configured to use a global certificate revocation checking. The configuration is located in `/var/lib/pki/pki-tomcat/conf/server.xml`. See link:https://github.com/dogtagpki/tomcatjss[Tomcat JSS]. This functionality requires a separate OCSP responder.
