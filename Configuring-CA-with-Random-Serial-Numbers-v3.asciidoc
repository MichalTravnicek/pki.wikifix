...
In case RSNv3 does not work as expected, follow this procedure to recover:

* Stop PKI server.
* Restore the `CS.cfg` from the backup.
* Remove new certificate records created with with RSNv3 from DS.
* Remove new certificate request records created with with RSNv3 from DS.
* Remove certificates created with with RSNv3 from user records in DS.
* Restart PKI server
* Revoke new certificates created with with RSNv3.

If the above procedure does not work, link:Restoring-PKI-Server[restore] PKI server from the backup.
