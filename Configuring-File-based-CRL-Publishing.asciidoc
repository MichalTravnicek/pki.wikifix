...
This page describes the process to configure file-based CRL publishing in CA.
...
= Enabling CRL Publishing =

To enable file-based CRL publishing configure the following parameters in `/var/lib/pki/pki-tomcat/ca/conf/CS.cfg`:
...
By default the CRL is only updated at scheduled times. To update the CRL on each certificate revocation:
