...
The signed audit configuration is stored in `/etc/pki/<instance>/<subsystem>/CS.cfg`:
...
By default the audit logs are stored in `/var/log/pki/<instance>/<subsystem>/signedAudit` with the following names:
...
See link:https://www.dogtagpki.org/wiki/Signed_Audit_Log[Signed Audit Log].
