...
By default the CI will perform tests using latest Fedora version and its previous version (e.g. Fedora 33 and 34). Sometimes it's necessary to test against specific Fedora versions. This can be done by creating a GitHub secred called `BASE64_MATRIX` to store the base64-encoded value of the test matrix. To reset the test matrix simply remove the secret.
