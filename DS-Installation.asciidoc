...
Once the DS instance is created, it's recommended to create a subtree (e.g. `dc=pki,dc=example,dc=com`)
to store all PKI subsystems:
...
See link:Enabling-SSL-Connection-on-DS[Enabling SSL Connection on DS].
