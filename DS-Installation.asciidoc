...
Once the DS instance is created, it's recommended to use a single subtree (e.g. `dc=pki,dc=example,dc=com`)
for all PKI subsystems such that eventually the subtree will look like the following:
...
See link:Enabling-SSL-Connection-in-DS[Enabling SSL Connection in DS].
