...
This document describes the process to deploy CA container on Podman with PKI 11.3 or later. The container image is available at link:https://quay.io/repository/dogtagpki/pki-ca[quay.io/dogtagpki/pki-ca].
...
** `ca_signing` certificate and key
** `ocsp_signing` certificate and key
** `audit_signing` certificate and key
** `subsystem` certificate and key
** `sslserver` certificate and key
