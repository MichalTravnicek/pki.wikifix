...
1. Install the DS CA certificate in PKI
2. Install a PKI-issued server certificate in DS (WIP)
...
----
$ certutil -L -d /etc/dirsrv/slapd-localhost -n Self-Signed-CA -a > ds-ca_signing.crt
----
...
----
$ certutil -A -d /etc/pki/pki-tomcat/alias -n "ds-ca_signing" -i ds-ca_signing.crt -t "CT,C,C"
----
...
= Installing PKI-Issued Server Certificate in DS (WIP) =
...
To generate a server certificate request in DS:
...
----
$ LDAPTLS_CACERT=ds-ca_signing.crt \
