...
$ certutil -R \
   ...
   -g 2048 \
   -Z SHA256 \
   -s "CN=server.example.com,O=EXAMPLE" \
...
Certificate Request:
    Data:
        Version: 1 (0x0)
        Subject: O = EXAMPLE, CN = server.example.com
...
* link:https://www.dogtagpki.org/wiki/Issuing_SSL_Server_Certificate_with_NSS[Issuing SSL Server Certificate with NSS]
