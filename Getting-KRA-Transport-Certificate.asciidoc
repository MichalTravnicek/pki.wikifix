...
This page describes the process to get KRA transport certificate.

For older versions see:

* link:Getting-KRA-Transport-Certificate-in-PKI-10.5[Getting KRA Transport Certificate in PKI 10.5]

= Getting KRA Transport Certificate from the Server Side =

To display KRA transport certificate:
...
To download KRA transport certificate:
...
----
$ pki kra-cert-transport-export --output-file transport.crt
----

To download KRA transport certificate chain:
...
To import KRA transport certificate into the user's NSS database (default is `~/.dogtag/nssdb`):

----
$ pki client-cert-import transport --cert transport.crt
----
