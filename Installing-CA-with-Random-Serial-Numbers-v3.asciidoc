...
If the `pki_cert_id_length` is not specified, it will use 160-bit certificate serial numbers by default, but currently NSS/JSS is only able to handle up to 159-bit serial numbers.
...
If the `pki_request_id_length` is not specified, it will use 160-bit request IDs by default.
