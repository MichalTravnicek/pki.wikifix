...
This page describes the process to install a KRA subsystem with existing certificates stored in a PKCS #12 file.

*Availability:* Since PKI 11.6.

= Preparing Certificate Chain =

Obtain the certificate chain from the CA (e.g. `ca_signing.crt`).
See link:Exporting-CA-System-Certificates[Exporting CA System Certificates].

= Preparing System Certificates =

Export KRA system certificates and the keys from an existing KRA subsystem into a PKCS #12 file (e.g. `kra-certs.p12`) and the CSRs into separate files.
See link:Exporting-KRA-System-Certificates[Exporting KRA System Certificates].
...
If the new KRA subsystem is being created on a different machine (i.e. migration), the existing subsystem and the SSL server certificates should not be included in the PKCS #12 file, or should be removed from the PKCS #12 file with the following commands:
