...
Export the system keys and certificates from the existing KRA into a PKCS #12 file with the following command:
...
To avoid conflicts with the existing KRA subsystem, the new KRA subsystem will use new SSL server and subsystem certificates,
...
Export the CSRs from the existing KRA with the following commands:
