...
Move the existing NSS database into `/etc/pki/pki-tomcat/alias` and store the password in `/etc/pki/pki-tomcat/password.conf` as follows:
...
It will install a KRA subsystem in the server instance and use the NSS database in `/etc/pki/pki-tomcat/alias`.
