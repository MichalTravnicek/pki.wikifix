...
The the key ID length (in bits) can be specified in `pki_key_id_length` parameter. By default it will be 160 bits.
...
The the key request ID length (in bits) can be specified in `pki_key_request_id_length` parameter. By default it will be 160 bits.
