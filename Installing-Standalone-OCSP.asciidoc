...
This page describes the process to install a standalone OCSP. In this scenario, the OCSP uses an external CA to issue the OCSP system certificates. It does not join the CA's security domain, and CRL publishing will need to be link:Configuring-OCSP-Publishing[configured] separately in the CA.
...
See also link:https://github.com/dogtagpki/pki/blob/master/docs/installation/ocsp/Installing_OCSP_with_External_Certificates.md[Installing OCSP with External Certificates].
