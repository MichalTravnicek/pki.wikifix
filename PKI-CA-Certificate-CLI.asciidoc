...
PKI provides CLI to manage certificates and certificate requests. See also link:Adding-System-User[Adding System User].

To manage certificate profiles, see link:PKI-CA-Profile-CLI[PKI CA Profile CLI].

To manage certificate requests, see link:PKI-CA-Certificate-Request-CLI[PKI CA Certificate Request CLI].
...
link:Generating-Certificate-Request[Generate a certificate request], for example:
...
See also link:Certificate-Profiles[Certificate Profiles].
...
See also link:https://www.dogtagpki.org/wiki/OCSPClient[OCSPClient].
