...
or wrapping a symmetric key with the KRA transport key.  There are three possible options here.  The method
...
the symmetric keys and wrappings are being done on a separate application, and this application does not interact with the KRA directly.

= Working with the Key Client = 

As shown in the code samples above, the end result is a `KeyClient` object which is defined in 
`KeyClient.java`.  Furthermore, in the source code in link:https://git.fedorahosted.org/cgit/pki.git/tree/base/kra/functional/src/com/netscape/cms/servlet/test/KRATest.java[KRATest.java], there are examples of the invocation
...
then wrapped using the public key in the transport cert of the KRA. Both the `transWrappedSessionKey` and `sessionWrappedPassphrase` are
...
which uses the client API to talk to the KRA.
