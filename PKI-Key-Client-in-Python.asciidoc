...
The Python Key Client is a client framework that allows you to easily interact with the KRA to archive, generate and recover secrets
...
The client uses python-requests to interact with the KRA.  All interactions with the KRA require
...
server does not interface with the KRA directly.
...
server then makes calls to the KRA.
...
`$PYTHON_PATH/pki/key.py`.  Furthermore, in the source code in `kratest.py`, there are examples of the invocation
