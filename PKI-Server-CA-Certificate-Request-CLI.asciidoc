...
----
$ pki-server ca-cert-request-import [--request <ID>] --csr <path to CSR> --profile <profile config filename>
----
...
* The `--profile` parameter specifies the filename of the profile configuration in `/etc/pki/pki-tomcat/ca` without the path (e.g. `serverCert.profile`).
