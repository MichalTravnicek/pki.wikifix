...
To generate a new key pair in HSM and an enrollment request:
...
To create temporary certificate:

----
$ pki-server cert-create <cert ID> --temp
----

To create temporary certificate with specific serial number:

----
$ pki-server cert-create <cert ID> --temp --serial <serial>
----

To rekey permanent certificate (not supported yet):
