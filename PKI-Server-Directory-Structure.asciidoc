...
* `pki-tomcatd`: This directory structure is used to run PKI server in Fedora's Tomcat which follows the link:http://www.pathname.com/fhs/[Filesystem Hierarchy Standard].
* `tomcat`: This directory structure is used to run PKI server in generic Tomcat environment. This directory structure is still under development.
