...
To update the TPS configuration, download the existing configuration into a file using the `tps-config-show` command above. Edit the file, then execute the following command:
