...
To configure OCSP responder to get the CRL from an LDAP server:
...
$ pki-server ca-config-set ca.publish.mapper.instance.LdapCrlMap.dnPattern cn=crl,dc=example,dc=com
...
$ pki-server ca-config-set ca.publish.rule.instance.LdapCrlRule.predicate issuingPointId==MasterCRL
