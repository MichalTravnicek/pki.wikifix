...
Before starting the build, create a BuildRoot override for all dependencies that have been built but not yet released. For example, if the new PKI 11.0.0-beta1 depends on JSS 5.0.0-beta1, then execute the following commands:
...
Finally, create a Bodhi update which includes the new build and the dependencies (if any), for example:
