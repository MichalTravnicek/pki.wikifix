...
Make sure the upstream project branch has a new version number to rebase to.
If not, update the version number in the upstream project branch.
...
Make sure the version/release number and `_phase` in the spec file (i.e. `pki.spec`) is higher than the latest one in link:https://koji.fedoraproject.org/koji/packageinfo?packageID=9999[Koji], for example:
...
Before creating the build, create a BuildRoot override for all dependencies that have been built but not yet released. For example, if the new PKI 11.0.0-beta1 depends on JSS 5.0.0-beta1, then execute the following commands:

```
$ bodhi overrides save jss-5.0.0-0.4.beta1.fc35
$ koji wait-repo f35-build --build=jss-5.0.0-0.4.beta1.fc35
```

*Note:* The BuildRoot override is not required for Rawhide.
...
*Note:* The Bodhi update is not required for Rawhide.
