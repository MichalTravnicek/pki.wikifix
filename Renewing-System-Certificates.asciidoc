...
First, identify the system certificates that are about to expire or have already expired with the following command:
...
If the SSL server certificate of the CA itself has already expired, disable the selftests with following command:

----
$ pki-server selftest-disable
----

Then create a temporary SSL server certificate to bring up the CA temporarily.
