...
If the CA subsystem is no longer working since the SSL server certificate of the CA itself has already expired, link:Creating-Temporary-SSL-Server-Certificate[create a temporary SSL server certificate], disable the selftests with following command:
...
Later follow the regular link:Renewing-SSL-Server-Certificate[renewal procedure] to issue the actual the SSL server certificate.
