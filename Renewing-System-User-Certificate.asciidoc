...
----
$ pki -U https://<hostname>:<secure port> -c <client NSS database password> -n <client certificate nickname> \
    client-cert-request <subject DN> --profile <renewal profile>
----

For example:

----
$ pki -U https://server.example.com:8443 -c Secret.123 -n testuser \
    client-cert-request uid=testuser --profile caManualRenewal
----

See link:https://www.dogtagpki.org/wiki/Certificate_Profiles[Certificate Profiles].

== Renewing expired user certificate ==

If the system user certificate has already expired, use the general link:https://www.dogtagpki.org/wiki/Certificate_Renewal[Certificate Renewal] procedure.
