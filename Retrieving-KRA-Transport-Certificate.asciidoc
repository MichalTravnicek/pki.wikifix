...
This page describes the process to retrieve KRA transport certificate.

= Displaying KRA Transport Certificate =
...
To retrieve KRA transport certificate:
...
= Retrieving KRA Transport Certificate Chain =

To retrieve KRA transport certificate chain:
...
In PKI 10.5 or older it's necessary to find the transport certificate by its CN attribute:
...
Then use the serial number to download the certificate into a file:
