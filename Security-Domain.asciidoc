...
A security domain is a service within a Certificate Authority to manage PKI service registration. You can elect to create a new security domain along with your self-signed Root CA. And each subsystem including Subordinate CA, TKS, OCSP, DRM, TPS must join a security domain within your enterprise. By jointing a security domain, the subsystem information such as services, host, and port will be made public to other subsystem. Each subsystem will also be issued a subsystem certificate which is signed by the security domain. Communications between the subsystem and the security domain will be done in client certificate authentication.
...
There will be 6 new groups created in the CA that hosts the security domain. They are as follows:

* Security Domain Administrators
** is responsible for managing user in the security domain 
