...
To migrate an existing CA from SSNv1 to SSNv2 it is possible to use the `pki-server` command `ca-range-generator-update`. The command takes the generator type to update to and the new generator name. So to update cert and request it is possible to run the commands:
