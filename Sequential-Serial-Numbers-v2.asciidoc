...
* Update the following parameters to hexedecimal:
...
* If there are no ranges available under the entry specified in `dbs.serialRangeDN` (default: `ou=certificateRepository,ou=ranges,<base dn>`) update the `nextRange` to `dbs.endSerialNumber + 1` and store in decimal in the entry specified in `dbs.serialDN` (default: `ou=certificateRepository,ou=ca,<base dn>`)
...
WARNING: The migration process must be done in parallel for all CA instances. Complete each migration step for all CA instances before proceeding to the next step.
