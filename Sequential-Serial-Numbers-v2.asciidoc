...
To update a running instance from SSNv1 to SSNv2 it is possible to use the `pki-server` command `ca-range-generator-update`. The command requires the generator type to update and the new geenrator. So to update cert and request it is possible to run the commands:
