...
If the project doesn't have the `.project` and `.classpath` files, it may be necessary to configure the project build path manually before the project can be used. Once these files are created, commit the files into the repository so others can reuse the files.
