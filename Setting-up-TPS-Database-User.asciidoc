= Overview =

This page describes the process to set up a user to access the TPS database in DS with PKI tools.

= Adding Database User =

----
$ pki-server tps-user-add \
    --full-name pkidbuser \
    --type agentType \
    pkidbuser
----

In PKI 11.6 or later the user certificate can be specified with the `--cert <path>` option.

= Assigning Certificate to Database User =

----
$ pki-server tps-user-cert-add \
    --cert /var/lib/pki/pki-tomcat/conf/certs/subsystem.crt \
    pkidbuser
----

= Granting Access to Database User =

----
$ pki-server tps-db-access-grant \
    uid=pkidbuser,ou=people,dc=tps,dc=pki,dc=example,dc=com
----
