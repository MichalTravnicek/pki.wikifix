...
This page describes audit events that are common in all PKI subsystems. The complete list of events, the message format, and the event description are stored in link:https://github.com/dogtagpki/pki/blob/master/base/server/cmsbundle/src/LogMessages.properties[LogMessages.properties].
...
Since version 10.5 a RANDOM_GENERATION event will be generated for any random number generated on the server (including link:https://www.dogtagpki.org/wiki/Random_Certificate_Serial_Numbers[Random Certificate Serial Numbers]). The event will include the information about the caller of the random number generator and the size of the random number in bytes.

By default only failed events will be logged. To log all events, change the link:https://www.dogtagpki.org/wiki/PKI_Server_Audit_Event_Filter[filter] in CS.cfg as follows:
...
If link:https://www.dogtagpki.org/wiki/Random_Certificate_Serial_Numbers[Random Certificate Serial Numbers] is enabled, there will be an additional RANDOM_GENERATION event:
...
* link:https://www.dogtagpki.org/wiki/PKI_Server_Audit_Event_Filter[PKI Server Audit Event Filter]
