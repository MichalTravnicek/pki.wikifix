...
When CA is started it will start a `SerialNumberUpdateTask` thread
...
When the CA creates a new cert or cert request, if there's no more available serial number in the range
it will switch to the new serial number range.

KRA also has a `KeyStatusUpdateTask` thread that does the same process for keys and key requests.

= OCSP =

When PKI server is started, OCSP will create a `DefStore` that stores the following information in memory:
...
OCSP will also start a `DefStoreCRLUpdater` thread that will periodically clear the CRL issuing point cache.

When OCSP receives a request, it will try to find the CRL issuing point in LDAP,
...
It will also start a `JobScheduler` thread that will wake up periodically and find jobs instances that match the schedule.
