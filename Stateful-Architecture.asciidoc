...
This page describes the current stateful architecture used in PKI 11.4.
In the future it might be converted into a stateless architecture.
...
When PKI server is started, the ACME responder will create an `LDAPConfigMonitor` thread
...
When the OCSP responder receives a request, it will try to find the CRL issuing point in LDAP,
