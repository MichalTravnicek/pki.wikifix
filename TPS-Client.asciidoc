...
----
$ ldapadd -h localhost -p 389 -D "cn=Directory Manager" -w Secret.123 << EOF
...
----
$ ldapsearch -x -D "uid=testuser,ou=people,dc=example,dc=com" -w Secret.123 -b "" -s base
----
...
op=var_set name=ra_host value=tps.example.com
...
* The `num_threads` can make the client spawn off many requests to the server at once.
* The `extensions=tokenType=userKey` is used to force the server to use the `userKey` token profile type.

----
op=ra_format uid=testuser pwd=Secret.123 new_pin=Secret.123 num_threads=1 extensions=tokenType=userKey
----

== Enrollment Operation ==

----
# Operation, note the num_threads, this can make the client spawn off many requests to the server at once.
# "extensions=tokenType=userKey, is used to force the server to use the "userKey" token profile type.
op=ra_enroll uid=testuser pwd=Secret.123 new_pin=Secret.123 num_threads=1 extensions=tokenType=userKey
----
...
* link:https://www.dogtagpki.org/wiki/Procedure_%27tpsclient%27[Procedure 'tpsclient']
