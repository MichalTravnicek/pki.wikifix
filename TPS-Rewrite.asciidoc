...
In the new implementation these operations are mapped into link:PKI-TPS-REST-API[PKI TPS REST API].
...
In the new implementation the services are written as separate REST resources using link:https://www.dogtagpki.org/wiki/RESTEasy[RESTEasy]. The authentication and authorization are handled by separate and configurable modules. The interface can be used by both CLI and UI which will be written separately.
